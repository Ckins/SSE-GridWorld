...
public class MazeBug extends Bug {
    ...
    private void increasePredict() {
        int curDir = getDirection();
        int to = calculateAngle(next, curDir);
        direction[to]++;
        usedDir = to;
    }
    
    private void decreasePredict() {
        direction[usedDir]--;
    }
    
    private Location makePredict(ArrayList<Location> array) {
        int curDir= getDirection();
        Location best = array.get(0);
        int i = 0;
        for (i = 0; i < array.size(); i++) {
            Location loc = array.get(i);
            int to = calculateAngle(loc, curDir);
            if (direction[to] > direction[i]) {
                best = loc;
            }
        }
        return best;
     ...
     */
    private int calculateAngle(Location loc, int direction) {
